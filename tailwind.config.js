/** @type {import('tailwindcss').Config} */
export default {
  darkMode: 'class',
  content: ['./index.html', './src/**/*.{js,ts,jsx,tsx}'],
  theme: {
    extend: {
      colors: {
        primary: {
          DEFAULT: 'var(--color-primary, #2563eb)',
          hover: 'var(--color-primary-hover, #1d4ed8)',
          foreground: 'var(--color-primary-foreground, #ffffff)',
        },
        neutral: {
          50: 'var(--color-neutral-50, #fafafa)',
          100: 'var(--color-neutral-100, #f5f5f5)',
          200: '#eaeaea',
          300: '#cccccc', 400: '#999999', 500: '#666666',
          600: '#444444', 700: '#333333', 800: '#222222', 900: '#111111',
        },
        success: {
          DEFAULT: 'var(--color-success, #10b981)',
          foreground: 'var(--color-primary-foreground, #ffffff)',
        },
        warning: {
          DEFAULT: 'var(--color-warning, #f59e0b)',
          foreground: 'var(--color-primary-foreground, #ffffff)',
        },
        danger:  {
          DEFAULT: 'var(--color-danger, #ef4444)',
          foreground: 'var(--color-primary-foreground, #ffffff)',
        },
        surface: 'var(--color-surface, #ffffff)',
        'surface-secondary': 'var(--color-surface-secondary, #fafafa)',
        'surface-tertiary': 'var(--color-surface-tertiary, #f5f5f5)',
        border: 'var(--color-border, #eaeaea)',
        'border-hover': 'var(--color-border-hover, #e5e5e5)',
        text: {
          primary: 'var(--color-text-primary, #111111)',
          secondary: 'var(--color-text-secondary, #666666)',
          tertiary: 'var(--color-text-tertiary, #999999)',
        },
        'on-surface': 'var(--color-text-primary, #111111)',
        'on-surface-variant': 'var(--color-text-secondary, #666666)',
        'primary-container': '#dbeafe',
        'on-primary-container': '#1e40af',
        'primary-fixed': '#eff6ff',
        'on-primary-fixed-variant': '#1d4ed8',
        'outline-variant': 'var(--color-border, #e2e8f0)',
        'surface-container-lowest': 'var(--color-surface-container-lowest, #ffffff)',
        'surface-container-low': 'var(--color-surface-container-low, #f8fafc)',
        'surface-container': 'var(--color-surface-container, #f1f5f9)',
        'surface-container-high': 'var(--color-surface-container-high, #e2e8f0)',
      },
      borderRadius: {
        sm: '0.375rem',      // 6px
        DEFAULT: '0.625rem', // 10px
        md: '0.625rem',      // 10px
        lg: '1rem',          // 16px
        xl: '1.25rem',
        '2xl': '1.5rem',
        full: '9999px',
      },
      spacing: {
        px: '1px',
        0: '0px',
        0.5: '0.125rem',
        1: '0.25rem',
        1.5: '0.375rem',
        2: '0.5rem',
        2.5: '0.625rem',
        3: '0.75rem',
        3.5: '0.875rem',
        4: '1rem',
        5: '1.25rem',
        6: '1.5rem',
        7: '1.75rem',
        8: '2rem',
        9: '2.25rem',
        10: '2.5rem',
        11: '2.75rem',
        12: '3rem',
        14: '3.5rem',
        16: '4rem',
        20: '5rem',
        24: '6rem',
        28: '7rem',
        32: '8rem',
        36: '9rem',
        40: '10rem',
        44: '11rem',
        48: '12rem',
        52: '13rem',
        56: '14rem',
        60: '15rem',
        64: '16rem',
        72: '18rem',
        80: '20rem',
        96: '24rem',
        xs: '0.5rem',
        sm: '0.75rem',
        md: '1rem',
        lg: '1.5rem',
        xl: '2rem',
        '2xl': '3rem',
        '3xl': '4rem',
        'section-gap-md': '3.5rem',
        'section-gap-lg': '5rem',
        gutter: '1.5rem',
      },
      fontSize: {
        'display': ['5rem', { lineHeight: '1.2', letterSpacing: '-0.04em', fontWeight: '300' }],
        'heading-1': ['2.5rem', { lineHeight: '1.2', letterSpacing: '-0.02em', fontWeight: '500' }],
        'heading-2': ['1.75rem', { lineHeight: '1.3', letterSpacing: '-0.02em', fontWeight: '500' }],
        'heading-3': ['1.25rem', { lineHeight: '1.4', letterSpacing: '-0.01em', fontWeight: '500' }],
        'body':      ['0.98rem', { lineHeight: '1.6', letterSpacing: '-0.01em', fontWeight: '400' }],
        'body-sm':   ['0.875rem', { lineHeight: '1.5', letterSpacing: '-0.01em', fontWeight: '400' }],
        'caption':   ['0.72rem', { lineHeight: '1.4', letterSpacing: '0.02em', fontWeight: '600' }],
        'meta':      ['12px', { lineHeight: '16px', letterSpacing: '0', fontWeight: '500' }],
        'display-lg': ['5rem', { lineHeight: '1.2', letterSpacing: '-0.04em', fontWeight: '300' }],
        'headline-lg': ['32px', { lineHeight: '40px', letterSpacing: '-0.03em', fontWeight: '600' }],
        'headline-lg-mobile': ['28px', { lineHeight: '36px', letterSpacing: '-0.03em', fontWeight: '600' }],
        'headline-md': ['24px', { lineHeight: '32px', letterSpacing: '-0.03em', fontWeight: '600' }],
        'headline-sm': ['20px', { lineHeight: '28px', letterSpacing: '-0.02em', fontWeight: '600' }],
        'body-lg': ['18px', { lineHeight: '28px', letterSpacing: '0', fontWeight: '400' }],
        'body-md': ['15px', { lineHeight: '22px', letterSpacing: '0', fontWeight: '400' }],
        'label-lg': ['16px', { lineHeight: '24px', letterSpacing: '0.01em', fontWeight: '500' }],
        'label-md': ['14px', { lineHeight: '20px', letterSpacing: '0.01em', fontWeight: '500' }],
        'label-sm': ['12px', { lineHeight: '16px', letterSpacing: '0.02em', fontWeight: '500' }],
      },
      fontFamily: {
        sans: ['Inter', 'system-ui', 'sans-serif'],
        mono: ['JetBrains Mono', 'monospace'],
        'display-lg': ['Plus Jakarta Sans', 'sans-serif'],
        'headline-md': ['Plus Jakarta Sans', 'sans-serif'],
        'headline-lg': ['Plus Jakarta Sans', 'sans-serif'],
        'headline-lg-mobile': ['Plus Jakarta Sans', 'sans-serif'],
        'body-md': ['Inter', 'sans-serif'],
        'body-lg': ['Inter', 'sans-serif'],
        'label-md': ['Inter', 'sans-serif'],
        'label-sm': ['Inter', 'sans-serif'],
      },
      boxShadow: {
        'card': '0 1px 3px 0 rgb(0 0 0 / 0.04), 0 1px 2px -1px rgb(0 0 0 / 0.06)',
        'card-hover': '0 4px 12px 0 rgb(0 0 0 / 0.08), 0 2px 4px -2px rgb(0 0 0 / 0.06)',
        'elevated': '0 10px 25px -5px rgb(0 0 0 / 0.08), 0 4px 10px -6px rgb(0 0 0 / 0.06)',
        'modal': '0 25px 50px -12px rgb(0 0 0 / 0.25)',
      },
      maxWidth: {
        'page': '1280px',
      },
      keyframes: {
        'fade-in': {
          from: { opacity: '0' },
          to: { opacity: '1' },
        },
        'fade-up': {
          from: { opacity: '0', transform: 'translateY(8px)' },
          to: { opacity: '1', transform: 'translateY(0)' },
        },
        'scale-in': {
          from: { opacity: '0', transform: 'scale(0.95)' },
          to: { opacity: '1', transform: 'scale(1)' },
        },
        'slide-in-right': {
          from: { opacity: '0', transform: 'translateX(12px)' },
          to: { opacity: '1', transform: 'translateX(0)' },
        },
      },
      animation: {
        'fade-in': 'fade-in 0.2s ease-out',
        'fade-up': 'fade-up 0.3s ease-out',
        'scale-in': 'scale-in 0.2s ease-out',
        'slide-in-right': 'slide-in-right 0.25s ease-out',
      },
    },
  },
  plugins: [],
}
